...
void Plane::set_tilt_wing_out()
{
    ...
    if(plane.control_mode == &plane.mode_ttwstabilize || plane.control_mode == &plane.mode_ttwhover || plane.control_mode == &plane.mode_ttwloiter){
        ...
        SRV_Channels::set_slew_rate(SRV_Channel::k_front_wing_tilt, g.flap_slewrate, 12000, G_Dt);
        SRV_Channels::set_slew_rate(SRV_Channel::k_back_wing_tilt, g.flap_slewrate, 12000, G_Dt);
        return;
    }
    ...
    //Yaw control with tilt wings test
        ...
        //float front_wing_tilt_percent = rc().find_channel_for_option(RC_Channel::AUX_FUNC::WING_TILT)->norm_input_ignore_trim();
        //float back_wing_tilt_percent = rc().find_channel_for_option(RC_Channel::AUX_FUNC::WING_TILT)->norm_input_ignore_trim();
        float wing_tilt_percent = rc().find_channel_for_option(RC_Channel::AUX_FUNC::WING_TILT)->norm_input_ignore_trim();

        //float front_wing_out  = constrain_float(front_wing_tilt_percent * 6000, -6000, 6000);
        //float back_wing_out = constrain_float(back_wing_tilt_percent * 6000, -6000, 6000);
        float wing_out = constrain_float(wing_tilt_percent * 6000, -6000, 6000);

        SRV_Channels::set_output_scaled(SRV_Channel::k_wing_tilt, wing_out);
        SRV_Channels::set_slew_rate(SRV_Channel::k_wing_tilt, g.flap_slewrate, 12000, G_Dt);
        
        SRV_Channels::set_slew_rate(SRV_Channel::k_front_wing_tilt, 0, 12000, G_Dt);
        SRV_Channels::set_slew_rate(SRV_Channel::k_back_wing_tilt, 0, 12000, G_Dt);

        

        //yaw input 
        float yaw_output = -abs(quadplane.get_desired_yaw_rate_cds(true) / (100*quadplane.command_model_pilot.get_rate()) * g.twing_yaw_max*100);//SRV_Channels::get_output_scaled(SRV_Channel::k_rudder) ;///45 *g.tilt_wing_yaw_max; //scale rudder input to max wing deflection for yaw

        float front_wing_ang = SRV_Channels::get_slew_limited_output_scaled(SRV_Channel::k_wing_tilt);
        float back_wing_ang = SRV_Channels::get_slew_limited_output_scaled(SRV_Channel::k_wing_tilt);


        float front_wing_out  = constrain_float(yaw_output + front_wing_ang, -6000, 6000);
        float back_wing_out = constrain_float(-yaw_output + back_wing_ang, -6000, 6000);
