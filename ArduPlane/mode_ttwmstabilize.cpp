...
bool ModeTTWMStabilize::_enter()
{
    quadplane.throttle_wait = false;
    return true;
}

void ModeTTWMStabilize::update()
{
...
void ModeTTWMStabilize::run()
{
    const uint32_t now = AP_HAL::millis();
    if (quadplane.tailsitter.in_vtol_transition(now)) {             //should I remove transitions?
    ...
    plane.stabilize_roll();             // no roll or pitch surfaces can be removed
    plane.stabilize_pitch();
    ...
    float front_wing_out  = constrain_float(front_wing_tilt_percent * 45, -4500, 4500);
    float back_wing_out = constrain_float(back_wing_tilt_percent * 45, -4500, 4500);
    ...
    // Center rudder
    output_rudder_and_steering(0.0);    // no rudder can be removed
}

// set the desired roll and pitch for a tailsitter
void ModeTTWMStabilize::set_tailsitter_roll_pitch(const float roll_input, const float pitch_input)
{
...
void ModeTTWMStabilize::set_limited_roll_pitch(const float roll_input, const float pitch_input)
{
